...
func TestReduceMatrix(t *testing.T) {
	...
	if newMatrix[0][0] != 0 {
		t.Errorf("value for [%d, %d] => %f, want %d", 0, 0, newMatrix[0][0], 0)
	}

	if newMatrix[0][1] != 1 {
		t.Errorf("value for [%d, %d] => %f, want %d", 0, 1, newMatrix[0][1], 1)
	}
	if newMatrix[1][0] != 10 {
		t.Errorf("value for [%d, %d] => %f, want %d", 1, 0, newMatrix[1][0], 10)
	}
	if newMatrix[1][1] != 11 {
		t.Errorf("value for [%d, %d] => %f, want %d", 1, 1, newMatrix[1][1], 11)
	}
}
...
func TestCalculateMeanValue(t *testing.T) {
	...
	if meanValue != 31 {
		t.Errorf("mean value for matrix => %f, want %d", meanValue, 31)
	}
}
